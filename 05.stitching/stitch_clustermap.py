...
def parse_tile_configuration(input_file):
    ...
    while line:
        if a > 3:
            match = re.match(r"^(?P<fov>.*).tif; ; \((?P<col>.*), (?P<row>.*), (?P<z>.*)\)", line)
...
tile_config_original = parse_tile_configuration(os.path.join(image_path, 'fused/configurations.txt'))
tile_config_registered = parse_tile_configuration(os.path.join(image_path, 'fused/configurations.registered.txt'))
...
# print(tile_config_without_blank)
...
for i in tqdm(range(tile_config_final.shape[0])):
    ...
    try:
        ...
        print(f"skiping {current_fov} due to : {e}")
        continue

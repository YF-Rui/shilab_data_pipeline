...
function sdata = rsf_workflow_example(config_path)
    ...
    for n=config.starting_fov_id:config.starting_fov_id + config.number_of_fovs - 1
        ...
        sdata = sdata.HistEqualize('reference_layer',config.ref_round,'reference_channel',4);
        ...
        % spot finding s
        sdata = sdata.SpotFinding('method',"max3d",'intensity_threshold', 0.1);%"spotiflow", 'probability_threshold', 0.01, 'intensity_threshold', 0.1);
        sdata = sdata.ReadsExtraction('voxel_size', [3 3 2]);

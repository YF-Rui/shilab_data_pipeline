...
function reads_stitching(config_path)
    ...
    tile_config_file = fullfile(image_path, "fused","configurations.registered.txt");
    ...
    for r=1:size(tile_config,1)
        ...
        fov_number = str2double(regexp(fov, '\d+', 'match', 'once'));
        fname_pattern = char(strcat(config.fov_id_pattern, "_goodSpots.csv"));
        current_spots_file = fullfile(signal_path, sprintf(fname_pattern, fov_number));

...
config_path = "F:/Xinlin_Gao/starfinder_test/workflow_parameters.json";
current_fov = 'tile_25';

config_raw = fileread(config_path);
if startsWith(config_raw, char(65279))
    config_raw = config_raw(2:end);
end
config = jsondecode(config_raw);

% add path for .m files
addpath(fullfile('F:/Github/starfinder-dev', 'code-base/src/'))

% create object instance
input_path = fullfile('F:/Xinlin_Gao/starfinder_test/', config.dataset_id, config.sample_id);
output_path = fullfile('F:/Xinlin_Gao/starfinder_test/', config.dataset_id, config.output_id);
...
% 
class(subtile_data)
isa(subtile_data, 'STARMapDataset')

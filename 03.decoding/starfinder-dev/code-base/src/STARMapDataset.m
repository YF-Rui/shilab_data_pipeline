...
% ...
    ...
    % ====Reads Calling====
        ...
        function obj = SpotFinding( obj, varargin )
            ...
            addOptional(p, 'probability_threshold', defaultProbabilityThreshold);
            ...
            input_channel_index = find(contains(string({ref_metadata(:).name}), "seq") == 1);
            ...
            switch p.Results.method
                ...
                case "spotiflow"
                    setenv('DISABLE_TQDM', '1');
                    obj.signal.allSpots = SpotFindingSpotiflow(input_img, p.Results.probability_threshold, p.Results.intensity_threshold);
            end
            ...
            if size(obj.signal.allSpots, 1) == 0
                ...
                if (p.Results.method == "spotiflow")
                    op = fullfile(obj.outputPath, "log", strcat(obj.fovID, '_spotiflow_prob_intensity.csv'));
                    prob_intens_table = obj.signal.allSpots(:, {'prob', 'MaxIntensity'});
                    writetable(prob_intens_table, op);
                end
        ...
        function obj = ReadsExtraction( obj, varargin )
            ...
            if ~isempty(obj.signal.allSpots)
                ...
                for current_layer=p.Results.layer
                    ...
                    fprintf(sprintf("Processing %s...", current_layer))
                    current_metadata = obj.metadata{current_layer}.ChannelInfo;
                    input_channel_index = find(contains(string({current_metadata(:).name}), "seq") == 1);
        ...
        function obj = ReadsFiltration( obj, varargin )
            ...
            addParameter(p, 'q_score_threshold', defaultthreshold);
            ...
            defaultSaveQscores = false;
            addParameter(p, 'save_qscore', defaultSaveQscores);
            ...
            if p.Results.save_qscore
                saveFolder = fullfile(obj.outputPath, "log", "q_scores");
                if ~exist(saveFolder, 'dir')
                    mkdir(saveFolder);
                end
                
                saveFile = fullfile(saveFolder, sprintf('%s_allSpots.csv', obj.fovID));
                %disp(obj.signal.allSpots(1:min(3, height(obj.signal.allSpots)), :));
                %coordsTable = obj.signal.allSpots(:, ["x", "y", "z","color_seq"]);
                writetable(obj.signal.allSpots, saveFile);
                fprintf('Saved decoded allSpots to %s\n', saveFile);
        ...
        function SaveEqualizedImages(sdata, outfolder)
            if ~exist(outfolder, 'dir')
                mkdir(outfolder);
            end
        
            l = sdata.layers.seq;  % histogram-equalized layers
            for li = 1:numel(l)
                layer = l(li);
                img_layer = sdata.images{layer};
                [Nx, Ny, Nz, Nch] = size(img_layer);
        
                for ch = 1:Nch
                    filename = fullfile(outfolder, sprintf('%s_channel%d.tif', layer, ch));
                    fprintf('Saving %s ...\n', filename);
        
                    for z = 1:Nz
                        slice = img_layer(:, :, z, ch);
        
                        % convert to uint16 for TIFF
                        slice_uint16 = uint16(slice);
        
                        if z == 1
                            imwrite(slice_uint16, filename, 'WriteMode', 'overwrite');
                        else
                            imwrite(slice_uint16, filename, 'WriteMode', 'append');
                        end
                    end
                end
            end
            fprintf('All equalized images saved to %s\n', outfolder);
        end

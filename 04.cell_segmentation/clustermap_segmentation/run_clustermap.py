...
import os, sys, json
sys.path.append("/media/shilab/ssd2tb/Xinlin_Gao/ClusterMap")
...
pi_path = os.path.join(config['input_path'], 'round1')
number_of_fovs = config['number_of_fovs']
starting_fov_id = config['starting_fov_id']
...
min_spot_per_cell = 20
cell_num_threshold = 0.1
window_size = 512

# Iterate through each tile
fovs = [f"Tile_{f:03d}" for f in range(starting_fov_id, number_of_fovs+starting_fov_id)]
for current_fov in fovs:
    ...
    # spots = spots.reindex(columns = ['spot_location_1', 'spot_location_2', 'spot_location_3', 'gene'])

    # Add gene code
    genes_df = pd.read_csv(os.path.join(config['input_path'], 'genes.csv'), header=None)
    ...
    print(len(np.unique(spots.gene)))
    # spots.gene = spots.gene.cat.reorder_categories((gene_order))


    spot_genes = set(spots.gene.cat.categories)
    ref_genes = set(gene_order)
    missing_in_ref = spot_genes - ref_genes
    missing_in_spots = ref_genes - spot_genes
    if missing_in_ref:
        print(f"Genes in spots but not in reference ({len(missing_in_ref)}): {list(missing_in_ref)[:10]}")
    if missing_in_spots:
        print(f"Genes in reference but not in spots ({len(missing_in_spots)}): {list(missing_in_spots)[:10]}")
    common_genes = list(spot_genes & ref_genes)
    gene_order_filtered = [g for g in gene_order if g in spot_genes]
    spots.gene = spots.gene.cat.reorder_categories(gene_order_filtered)
